      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-4">
            Our <span className="bg-gradient-to-r from-primary to-primary-dark bg-clip-text text-transparent">Products</span>
          </h2>
          
          <motion.div
            className="w-24 h-1 bg-gradient-to-r from-primary to-primary-dark mx-auto mb-6"
            initial={{ width: 0 }}
          ...
          >
            <Link
              href="/products"
              className="inline-flex items-center gap-2 bg-gradient-to-r from-primary to-primary-dark hover:from-primary-dark hover:to-primary text-white px-8 py-4 rounded-full font-semibold transition-all duration-300 shadow-lg hover:shadow-xl group"
            >
                ...
                >
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-12">
                      {/* Badge */}
                      <div className="mb-4">
                        <span className="inline-block bg-primary text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg backdrop-blur-sm">
                          {featuredProducts[0].category?.name || "Featured"}
                        </span>
                      </div>

                      <h3 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4 group-hover:text-primary transition-colors duration-300">
                  ...
                  {featuredProducts.slice(1, 5).map((product, index) => (
                    ...
                    >
                      <Link
                        ...
                        className="group relative block h-full min-h-[140px] md:min-h-[150px] rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-200/50 hover:border-primary/50"
                      >
                        <div className="absolute inset-0">
                          <Image
                        ...
                        <div className="absolute inset-0 flex items-center justify-between p-4 md:p-5">
                          <div className="flex-1">
                            <span className="text-primary text-xs font-bold mb-1 uppercase tracking-wider block">
                              {product.category?.name || "Featured"}
                            </span>
                            <h3 className="text-lg md:text-xl font-bold text-white mb-1 group-hover:text-primary transition-colors line-clamp-1">
                              {product.name}
                            </h3>
                          ...
                          >
                            <div className="bg-white/10 backdrop-blur-sm hover:bg-white/20 rounded-full p-2 border border-white/20 group-hover:border-white/40 transition-all duration-300">
                              <ArrowRight className="w-4 h-4 md:w-5 md:h-5 text-white group-hover:text-primary transition-colors" />
                            </div>
                          </motion.div>
                        </div>
                      </Link>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {regularProducts.map((product, index) => (
                ...
                >
                  <Link
                    href={`/products/${product.slug}`}
                    className="block bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 hover:border-primary/50 h-full"
                  >
                    ...
                    <div className="relative h-56 overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200">
                      ...
                      <div className="absolute top-4 right-4">
                        <span className="bg-primary text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg backdrop-blur-sm">
